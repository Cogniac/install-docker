...
nvidia_driver_version="440.118.02"
...
install_nvidia() {
    ...
    if [ ! -n "${have_nvidia}" ]; then
        ...
        if [ -f ${nvidia_driver_installer} ]; then
            ...
            $sh_c "./${nvidia_driver_installer} --dkms --silent --no-drm --no-opengl-files --no-backup"
            nvidia-smi
            ...
            sudo apt-get install --no-install-recommends -y nvidia-container-runtime
            create_docker_daemon
