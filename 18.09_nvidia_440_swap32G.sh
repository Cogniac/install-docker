...
# if modifying docker version, ensure corresponding nvidia-docker2 and container runtimes are 
# available.
...
add_swap() {
    if [[ -n $(swapon -s) ]]; then 
        ...
        echo "Creating swap - size ${swap_size}"
        $sh_c "fallocate -l ${swap_size} /swapfile"
...
install_nvidia() {
    have_nvidia=`lspci | grep NVIDIA`
    if [ ! -n "${have_nvidia}" ]; then
        echo "Nvidia drivers not being installed"
    else
        $sh_c "apt -y install build-essential dkms"
        dpkg -l | grep -i nvidia  | awk -F ' ' '{print $2}' | xargs sudo apt-get purge -y
        if [ ! -f ${nvidia_driver_installer} ]; then
            wget http://us.download.nvidia.com/tesla/${nvidia_driver_version}/${nvidia_driver_installer}
        fi
        if [ -f ${nvidia_driver_installer} ]; then
            $sh_c "chmod u+x ./${nvidia_driver_installer}"
            $sh_c "./${nvidia_driver_installer} --dkms --silent"
            nvidia-smi
            $sh_c "nvidia-persistenced --persistence-mode"
            $sh_c "curl -s -L https://nvidia.github.io/nvidia-docker/gpgkey |   sudo apt-key add -"
            distribution=$(. /etc/os-release;echo $ID$VERSION_ID)
            $sh_c "curl -s -L https://nvidia.github.io/nvidia-docker/$distribution/nvidia-docker.list |   sudo tee /etc/apt/sources.list.d/nvidia-docker.list"
            $sh_c "apt update"
            nvidia_docker2_version=`apt-cache madison nvidia-docker2 | grep ${docker_version} | head -n 1 | cut -d ' ' -f 3`
            nvidia_runtime_version=`apt-cache madison nvidia-container-runtime | grep ${docker_version} | head -n 1 | cut -d ' ' -f 3`
            if [[ -z $nvidia_docker2_version ]] || [[ -z $nvidia_runtime_version ]]; then
                echo "Unable to find nvidia-docker2 or nvidia-container-runtime for docker version ${docker_version}"
            else 
                echo "Nvidia docker2: ${nvidia_docker2_version}"
                echo "Nvidia runtime: ${nvidia_runtime_version}"
                $sh_c "apt-get install -y -q nvidia-docker2=${nvidia_docker2_version} nvidia-container-runtime=${nvidia_runtime_version}"
                create_docker_daemon
                $sh_c "mv daemon.json /etc/docker"
            fi
        fi 
    fi
}
	...
	case "$lsb_dist" in
		ubuntu|debian)
			...
			(
				...
				$sh_c 'apt-get update'
                # explicitly install ce-cli otherwise it will drift away from docker-ce version which presents a problem for nvidia-docker2
